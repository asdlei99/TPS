...
/***********************<< 向施耐德电气系统发送任意操作指令 >>************************	
 *[消息体]			 
 *    + s8  写入施耐德电气控制串口的操作指令 施耐德消息命令长度均为8
 *[消息方向]
 *
 *   CNS => CnDevice
 ...
 * +EmCnAction emCnStart 开始 emCnStop 停止
 *[消息方向]
 *   
 *      ui => cndevice 
 ...
 *[消息体]			 
 *   EmExpMode   
 *   BOOL      FALSE 设置失败  TRUE 成功
 *   u8        当前位置索引
 *[消息方向]
 *   
 *   cndevice => ui
 */_event(  ev_Cn_DCamAutoExposureInd )
   _body (  EmExpMode,               1)
   _body (  BOOL     ,               1)
   _body (  u8       ,               1)
   _ev_end
 ...
 *[消息体]
 *
 * EmDCamGain  增益范围 0dB  3dB  6dB ···42dB
 *
 *[消息方向]
 *  
 *   ui => cndevice 
...
/***********************<< 文档摄像机设置开机调用预置位 >>*********************	
 *[消息体]
 * BOOL         =TRUE 开机调用预置位 =FALSE 关闭调用 
 *
 *[消息方向]
 *   
 *   cndevice => ui 
 */_event(  ev_Cn_SetDCamPreset_Cmd)
   _body (  BOOL	   ,        1  )
   _ev_end
 ...
 *   
 *      cnc => cns 
 */_event(  ev_Cn_DocSetSnapShotCmd )
   _ev_end
 ...
 *   
 *      cnc => cns 
 */_event(  ev_Cn_DocSetSnapShotInd )
   _body (BOOL ,         1          )
   _ev_end
 ...
 *   
 *   cndevice => tptool 
 */_event(  ev_Cn_SetDCamPreset_Ind)
   _body (BOOL, 1)
   _body (BOOL, 1)
   _body (u8  , 1)
   _ev_end
...
/***********************<< cndevice将待机信息通知cnue >>************************	
 *[消息体]		
 *BOOL		=TRUE 待机状态 唤醒 =FALSE 待机
 *BOOL		=TRUE 会议空闲      =FALSE 在会
 *
 *[消息方向]  
 * CnDevice => Cnue
 */_event(  ev_Cn_DeviceSleepInfo_Nty)
   _body (BOOL, 1)
   _body (BOOL, 1)
   _ev_end
 ...
 */
   ...
   _event(  ev_cns_centreTVModeTimer  )
   _ev_end 
    /**************<< 通知CNC\TPAD\TPTOOL当前文档摄像机索引  >>******************* 
 *[消息体]     
 *    + u8    当前摄像机索引
 *
 *[消息方向]
 ...
 *   
 *      cnc => cns 
 */_event(  ev_Cn_SetDocRecallPresetTimerCom6  )
   _ev_end
   /**********************<< 回复摄像机位置 >>*************************  
 *[消息体]
 *  
 ...
 *   
 *      cnc => cns 
 */_event(  ev_Cn_SetDocRecallPresetTimerCom5  )
   _ev_end
