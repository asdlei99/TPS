...
#ifdef KD_EXT
# define SSL_OP_NO_GMSSLv1                               0x40000000U
#endif /** KD_EXT */
...
#ifdef KD_EXT
#  define QT_MAX_IDENTITY_LEN 128
typedef unsigned int (*SSL_qt_client_cb_func)(SSL *ssl,
                                               const char *peer_id,
                                               char *local_id,
                                               unsigned int max_local_id_len,
                                               unsigned char *qk,
                                               unsigned int max_qk_len);

typedef unsigned int (*SSL_qt_server_cb_func)(SSL *ssl,
                                               const char *id,
                                               unsigned char *qk,
                                               unsigned int max_qk_len);


void SSL_CTX_set_qt_client_callback(SSL_CTX *ctx, SSL_qt_client_cb_func cb);
void SSL_CTX_set_qt_server_callback(SSL_CTX *ctx, SSL_qt_server_cb_func cb);

int SSL_CTX_use_qt_identity(SSL_CTX *ctx, const char *identity);
const char *SSL_get_local_identity(const SSL *s);
const char *SSL_get_peer_identity(const SSL *s);
#endif /** KD_EXT */
