...
#define INVALID_ALIAS_FOR_SHOW        _T("; / ? : @ & = + $，% ^ ——#" )           //提示给用户的非法字符，别名中不可以包含，否则会引起呼叫失败
#define INVALID_ALIAS_FOR_CODE        _T(" `~!@$^&%()+=#|{}:;'\\[]<>/?！……（）——【】‘；：”“'。，、？￥＄＆") //编码中使用，为了便于提示，同时禁用掉中文输入法下的相同字符
#define INVALID_ALIAS_FOR_FLODER      _T("\\/:*?<>|")
...
#define  ERR_UMC_LOGIN_UMS_SYSINITING errLoginUmsSysIniting   
   ...
   //达到用户登录上限
   errLoginUmslocking,
#define  ERR_UMC_LOGIN_UMS_LOCKING errLoginUmslocking   
