...
namespace DuiLib
{
	...
	void CContainerUI::SetInset(RECT rcInset)
	{
		...
		if ( CPaintManagerUI::IsSelfAdaption() )
		{
			...
			m_rcInset.left = rcInset.left * fAdpX;
			m_rcInset.right = rcInset.right * fAdpX; 
			m_rcInset.top = rcInset.top * fAdpY;
			m_rcInset.bottom = rcInset.bottom * fAdpY; 
		}
		else
		{
			m_rcInset = rcInset;
	...
	void CContainerUI::DoEvent(TEventUI& event)
	{
		...
		if( (event.Type == UIEVENT_BUTTONDOWN || event.Type == UIEVENT_RBUTTONDOWN /*|| event.Type == UIEVENT_GESTURENOTIFY */) && m_bContainerClick) 
		{
			if( m_pManager != NULL && IsEnabled()) m_pManager->SendNotify(this, DUI_MSGTYPE_BUTTONDOWN);
			return;
		}
                ...
                if ( m_pHorizontalScrollBar->IsVisible() )
                {
                    ...
                    else
                    {
                        ...
                        if( m_pHorizontalScrollBar->GetScrollRange() != cxScroll ) {
                            int iScrollPos1 = m_pHorizontalScrollBar->GetScrollPos();
                            m_pHorizontalScrollBar->SetScrollRange(::abs(cxScroll));
                            if( m_pHorizontalScrollBar->GetScrollRange() == 0 ) {
                                m_pHorizontalScrollBar->SetVisible(false);
                                m_pHorizontalScrollBar->SetScrollPos(0);
                            }
                            if( iScrollPos1 > m_pHorizontalScrollBar->GetScrollPos() ) {
                                SetPos(m_rcItem);
                            }
                        }
                    }                  
	...
	void CContainerUI::SetContainerClick( bool bclick /*= false*/ )
	{
