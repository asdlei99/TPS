...
namespace DuiLib
{
	...
	void  COptionUI::SetCheckNoMsg(bool bCheck)
	{
		if( m_bSelected == bCheck ) 
			return;
		m_bSelected = bCheck;
		if( m_bSelected ) 
	...
	void COptionUI::SetEnabled(bool bEnable)
	{
		...
		if( !IsEnabled() ) {
			if( m_bSelected ) m_uButtonState = UISTATE_SELECTED | UISTATE_DISABLED;
	...
	void COptionUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
	{
		...
		else if( _tcscmp(pstrName, _T("selectednomsg")) == 0 ) SetCheckNoMsg(_tcscmp(pstrValue, _T("true")) == 0);
	...
	void COptionUI::PaintText(HDC hDC)
	{
		if( (m_uButtonState & UISTATE_SELECTED) != 0 )
		{
			...
			if( m_bShowHtml )
				...
				CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rc, m_sText, IsEnabled()?m_dwTextColor:m_dwDisabledTextColor, \
