...
void CComboWnd::Init(CComboUI* pOwner)
{
    ...
    RECT rc = rcOwner;
    rc.top = rc.bottom + m_pOwner->GetPopBoxInterspace();// 父窗口left、bottom位置作为弹出窗口起点
    rc.bottom = rc.top + szDrop.cy;	// 计算弹出窗口高度
    if( szDrop.cx > 0 ) rc.right = rc.left + szDrop.cx;	// 计算弹出窗口宽度

	int cyFixed = 0;
	if (m_pOwner->GetItemHeight() != 0)
	{
		cyFixed = pOwner->GetCount() * pOwner->GetItemHeight();
	...
	RECT r=m_pOwner->GetInset();
	rc.right = rc.right + m_pOwner->GetInset().right + m_pOwner->GetInset().left;
	rc.bottom = rc.bottom + m_pOwner->GetInset().bottom + m_pOwner->GetInset().top;
	m_sz.cx = rc.right - rc.left ;
	m_sz.cy = rc.bottom - rc.top ;
...
LRESULT CComboWnd::HandleMessage(UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    if( uMsg == WM_CREATE ) {
		...
		if (m_pOwner->GetMosPopBoxBk()!= NULL)
		{
			m_pLayout->SetBkImage(m_pOwner->GetMosPopBoxBk()/*_T("res\\public\\combo"*/);
			m_pLayout->SetAttribute(_T("isvermosaicbk"),_T("true"));

			m_pLayout->SetInset(RECT(m_pOwner->GetInset()));
			int cx = m_sz.cx;
			int cy = m_sz.cy;
			...
			m_pLayout->SetFixedHeight(cy);
			m_pLayout->SetFixedWidth(cx);
        ...
        m_pLayout->EnableScrollBar(m_pOwner->GetVerticalScrollBar() ,m_pOwner->GetHorizontalScrollBar());
...
CComboUI::CComboUI() : m_pWindow(NULL), m_iCurSel(-1), m_uButtonState(0),m_iInterspace(0)
{
...
{
	// fix combobox下拉框 滚轮/上下按键 下拉框关闭的问题 by zh. 20140718
     if( m_pWindow != NULL ) m_pWindow->Close();
    if( (iIndex == m_iCurSel) && IsChangeComboText() ) 
        return true;
    ...
    if( !pControl /*|| !pControl->IsVisible() || !pControl->IsEnabled()*/ ) return false; //没显示出来也切换值 -xcr 20151024
    IListItemUI* pListItem = static_cast<IListItemUI*>(pControl->GetInterface(_T("ListItem")));
    if( pListItem == NULL ) return false;
    m_iCurSel = iIndex;
    if( m_pWindow != NULL || bTakeFocus ) pControl->SetFocus();
    if ( IsChangeComboText() )//此处调Select会死循环
    {
...
void CComboUI::DoEvent(TEventUI& event)
{
    ...
    {
		if(!IsEnabled()) return;
		//fix bug 参照windows默认combo效果 下拉随框滚滚动 by zh. 20140718
		bool bDownward = LOWORD(event.wParam) == SB_LINEDOWN;
		if(m_pWindow != NULL) m_pWindow->Scroll(0,(bDownward ? 10: -10 ));
		else SelectItem(FindSelectable(m_iCurSel + (bDownward ? 1 : -1), bDownward));
        return;
    }
...
void CComboUI::SetDropBoxSize(SIZE szDropBox)
{
	if ( CPaintManagerUI::IsSelfAdaption() )
	{
		float fAdpX, fAdpY;
		CPaintManagerUI::GetAdpResolution( &fAdpX, &fAdpY );
		m_szDropBox.cy = szDropBox.cy * fAdpY;
		m_szDropBox.cx = szDropBox.cx * fAdpX;
	}
	else
	{
...
void CComboUI::SetAttribute(LPCTSTR pstrName, LPCTSTR pstrValue)
{
	...
	else if( _tcscmp(pstrName, _T("MosPopBoxBk")) == 0 ) SetMosPopBoxBk(pstrValue);
	else if( _tcscmp(pstrName, _T("dropboxInterspace")) == 0 ) SePopBoxInterspace(_ttoi(pstrValue));
	else if( _tcscmp(pstrName, _T("itemheight")) == 0 ) SetItemHeight(_ttoi(pstrValue));
    else if( _tcscmp(pstrName, _T("changecombotext")) == 0 ) SetChangeComboText(_tcscmp(pstrValue, _T("true")) == 0);
    else if( _tcscmp(pstrName, _T("gdiplus")) == 0 ) SetGdiPlus(_tcscmp(pstrValue, _T("true")) == 0);
...
void CComboUI::PaintText(HDC hDC)
{
    ...
    if( m_iCurSel >= 0 && IsChangeComboText() ) 
    {
        CControlUI* pControl = static_cast<CControlUI*>(m_items[m_iCurSel]);
        ...
        if( pElement != NULL ) {
			 ...
			 {
				 CListLabelElementUI* pitem = static_cast<CListLabelElementUI*>(pElement);
				 CDuiString str =pitem->GetText();

                if( m_bGdiPlus )
                {
                     CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rcText, str, m_dwTextColor, \
                         m_ListInfo.nFont, DT_SINGLELINE | m_ListInfo.uTextStyle);
                }
                else
                {
				     CRenderEngine::DrawText(hDC, m_pManager, rcText, str, m_dwTextColor, \
					     m_ListInfo.nFont, DT_SINGLELINE | m_ListInfo.uTextStyle);
                }
			 }
			 else
	...
	else
	{
        if( m_bGdiPlus )
        {
            CRenderEngine::DrawTextByGdiPlus(hDC, m_pManager, rcText, m_sText, m_dwTextColor, \
                m_ListInfo.nFont, DT_SINGLELINE | m_ListInfo.uTextStyle);
        }
        else
        {
...
bool CComboUI::IsGdiPlus()
{
    return m_bGdiPlus;
}

void CComboUI::SetGdiPlus(bool bGdiPlus)
{
    if( m_bGdiPlus == bGdiPlus ) return;

    m_bGdiPlus = bGdiPlus;
    Invalidate();
}

void CComboUI::SetItemHeight( int dwHeight )
{
	if ( CPaintManagerUI::IsSelfAdaption() )
	{
		float fAdpX, fAdpY;
		CPaintManagerUI::GetAdpResolution( &fAdpX, &fAdpY );
		m_dwItemHeight = dwHeight * fAdpY;
	}
	else
	{
		m_dwItemHeight = dwHeight;
	}
}
