...
void CTouchListUI::SetScrollPos(SIZE szPos)
{
    int cx = 0;
    int cy = 0;
...
/*
    ...
    if (cx != 0)
    {
        if(GetListHeadNameH().IsEmpty() == false)
        {
            CTouchListUI* pListHeadH = static_cast<CTouchListUI*>(GetManager()->FindControl(GetListHeadNameH()));
            if(pListHeadH)
            {
                RECT rcPos;
                for( int it2 = 0; it2 < pListHeadH->m_items.GetSize(); it2++ ) {
                    CControlUI* pControl = static_cast<CControlUI*>(pListHeadH->m_items[it2]);
                    if( !pControl->IsVisible() ) continue;
                    if( pControl->IsFloat() ) continue;
    ...
    if (cy != 0)
    {
        if(GetListHeadNameV().IsEmpty() == false)
        {
            CTouchListUI* pListHeadV = static_cast<CTouchListUI*>(GetManager()->FindControl(GetListHeadNameV()));
            if(pListHeadV)
            {
                RECT rcPos;
                for( int it2 = 0; it2 < pListHeadV->m_items.GetSize(); it2++ ) {
                    CControlUI* pControl = static_cast<CControlUI*>(pListHeadV->m_items[it2]);
                    if( !pControl->IsVisible() ) continue;
                    if( pControl->IsFloat() ) continue;
...
void CTouchListUI::SetAttribute( LPCTSTR pstrName, LPCTSTR pstrValue )
{
    ...
    else if( _tcscmp(pstrName, _T("listdrag")) == 0)  SetListDrag( pstrValue );
    else if( _tcscmp(pstrName, _T("listheadh")) == 0)  SetListHeadNameH( pstrValue );
    else if( _tcscmp(pstrName, _T("listheadv")) == 0)  SetListHeadNameV( pstrValue );
...
void CTouchListUI::SetListDrag( LPCTSTR pIsListDrag )
{
    if (( _tcscmp(_T("false"),pIsListDrag) ==0 )||( _tcscmp(_T("FALSE"),pIsListDrag) ==0))
    {
        m_bListDrag = false;
    }
    else
...
bool CTouchListUI::IsListDrag() const
{
    return m_bListDrag;
}

CDuiString CTouchListUI::GetListHeadNameH() const
{
    return m_sListHeadNameH;
}
void CTouchListUI::SetListHeadNameH(CDuiString sListHeadNameH)
{
    m_sListHeadNameH = sListHeadNameH;
}
CDuiString CTouchListUI::GetListHeadNameV() const
{
    return m_sListHeadNameV;
}
void CTouchListUI::SetListHeadNameV(CDuiString sListHeadNameV)
{
    m_sListHeadNameV = sListHeadNameV;
}
