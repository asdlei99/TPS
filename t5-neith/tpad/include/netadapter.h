...
typedef struct tagIpConfig
{
    ...
    u32  dwPriorityCns;   //首选dns服务器
    u32  dwBkupCns;       //备用dns服务器
public:
...
public:
    ...
    static BOOL GetNetAdapterName(); //获取当前网卡
	...
	BOOL    NetIpConflict( DWORD chIP ); //ip冲突检测,TRUE为冲突
	EmNetStatus    NetAlive( DWORD dw );  //当前网络断链与否,TRUE为正常
	...
	BOOL    SetDHCPToPC( BOOL bRefreshIPCfg = TRUE, BOOL bWifi = FALSE ); //正确返回TRUE，设置IP使用DHCP
    
    BOOL    SetDnsDhcpToPc(  BOOL bRefreshIPCfg = TRUE, BOOL bWifi = FALSE );   //设置DNS服务器使用DHCP
    
    BOOL    SetDnsAddr( u32 dwDNSAddr , BOOL bRefreshIPCfg = FALSE, BOOL bWifi = FALSE );      //设置DNS地址
    BOOL    SetBackupDnsAddr( u32 dwDNSAddr , BOOL bRefreshIPCfg = FALSE, BOOL bWifi = FALSE ); //设置备用DNS地址

    BOOL    DeleteDnsAddr( u32 dwDNSAddr, BOOL bWifi = FALSE ); //删除DNS地址
...
private:
    ...
    static CString      m_strWifiNetCardName; //当前无线网卡名称
public:
	void UpdateUI();
	BOOL GetIfData();
