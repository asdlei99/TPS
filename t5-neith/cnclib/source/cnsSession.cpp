...
u16 CCnsSession::DisconnectCns()
{
    ...
    {
		u16 ret = dynamic_cast<CCncAddrbook*>(m_pAddrBookIF)->UnRegAddrBook();
		if (ret != NO_ERROR)
		{
			u32 dwNode = g_CncApp.GetNodeId();
			u16 wRet = OspPost(MAKEIID(GetAppID()), OSP_DISCONNECT, (const void*)&dwNode, sizeof(u32));
		}
	}	
...
u16 CCnsSession::SetTpadSysName()
{
	ZeroMemory(&m_cMsg, sizeof(CMessage));
	m_cMsg.event = ev_TPadSetSysName_Nty;	
	u16 wRet = PostMsg( TYPE_CMESSAGE );

    PrtMsg( ev_TPadSetSysName_Nty, emEventTypeCnsSend, "cnc对系统名进行了修改");
    return wRet;
}
...
{
	REG_PFUN( OSP_DISCONNECT, CCnsSession::OnLinkBreak );
	REG_PFUN( ev_CnLogin_Rsp, CCnsSession::OnCnsLoginRsp );
	REG_PFUN( ev_CnList_Nty, CCnsSession::OnCnsListNty );
	REG_PFUN( ev_CnLoginIns_Rsp, CCnsSession::OnLoginCnsInsRsp );
	REG_PFUN( ev_tpRoomOffline_Nty, CCnsSession::OnCnsOfflineNty );
	REG_PFUN( ev_tpRoomOnline_Nty, CCnsSession::OnCnsOnlineNty );
	REG_PFUN( ev_CNSLoginByOther_Notify, CCnsSession::OnLoginByOtherNotify );

    REG_PFUN( ev_Cn_CentreSleepNty, CCnsSession::OnCentreSleepNty ); //中控待机消息

    REG_PFUN( ev_TPadLoginInfo_Nty, CCnsSession::OnTPadLoginUserNotify );
    REG_PFUN( ev_TPadHomeKey_Nty, CCnsSession::OnTPadHomeKeyNotify );
	REG_PFUN( ev_OpenCenterCtrl_Nty, CCnsSession::OnTPadOpenCenterCtrl );
	REG_PFUN( ev_OpenSysConfig_Nty, CCnsSession::OnTPadOpenSysConfig );
    REG_PFUN( ev_OpenCnsManage_Nty, CCnsSession::OnTPadOpenCnsManage );
	REG_PFUN( ev_WBConfStateNty, CCnsSession::OnWBConfStateNty );
	REG_PFUN( ev_tpp_NewConfCall_Req, CCnsSession::OnNewConfCallReq );
	REG_PFUN( ev_CnsDisconnect_Nty, CCnsSession::OnTPadCnsDisconnectNty );
}
...
// 	TLoginRet tLogRe = *reinterpret_cast<TLoginRet *>(cTpMsg.GetBody());
	...
	PrtMsg( ev_CnLogin_Rsp, emEventTypeCnsRecv,"EmTpLoginUmsRet: %d, ErrorCode: %d, ErrorNumber: %d ", tLogRe.GetRet(), tLogRe.m_dwErrorCode,tLogRe.m_byNumber );
	m_tLogRe = tLogRe;//记录登录返回结果
    ...
    case tp_LoginUms_SysIniting: //系统正在初始化
		...
		break;
	case tp_LoginUms_locking: //账号被锁定
		bLogin = FALSE;
		dwErrCode = ERR_UMC_LOGIN_UMS_LOCKING;
		break;
	default:
		bLogin = FALSE;
...
void CCnsSession::OnTPadOpenCenterCtrl( const CMessage& cMsg )
{
	PrtMsg( ev_OpenCenterCtrl_Nty, emEventTypeTPadRecv, "" );

	PostEvent( UI_TPAD_SHOWCENTRECTRL_NTY );
}

void CCnsSession::OnTPadOpenSysConfig( const CMessage& cMsg )
{
	PrtMsg( ev_OpenSysConfig_Nty, emEventTypeTPadRecv, "" );

	PostEvent( UI_TPAD_SHOWSYSCONFIG_NTY );
}

void CCnsSession::OnTPadOpenCnsManage( const CMessage& cMsg )
{
    PrtMsg( ev_OpenCnsManage_Nty, emEventTypeTPadRecv, "" );

    PostEvent( UI_TPAD_SHOWCNSMANAGE_NTY );
}

void CCnsSession::OnWBConfStateNty( const CMessage& cMsg )
{
	bool bIsInWBConf = *reinterpret_cast<bool *>( cMsg.content );

	PrtMsg( ev_WBConfStateNty, emEventTypeTPadRecv, "IsInWBConf:%d", bIsInWBConf );

	PostEvent( UI_WBCONFSTATE_NTY, (WPARAM)bIsInWBConf );
}

void CCnsSession::OnNewConfCallReq( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg); 

	TTPCnsConfStatus tPCnsConfStatus = *reinterpret_cast<TTPCnsConfStatus*>(cTpMsg.GetBody());

	PrtMsg( ev_tpp_NewConfCall_Req, emEventTypeCnsRecv, "WB新会议呼进请求Tpad加入" );

	PostEvent( UI_NEWWBCONFCALL_REQ, 0, 0  );
}

void CCnsSession::OnTPadCnsDisconnectNty(const CMessage& cMsg)
{
	BOOL bActiveExit = *reinterpret_cast<BOOL*>( cMsg.content );

	PrtMsg( ev_CnsDisconnect_Nty, emEventTypeTPadRecv, "bActiveExit:%d", bActiveExit );

	PostEvent( UI_TPAD_CNSDISCONNECTED_NTY, (WPARAM)bActiveExit );
}

u16 CCnsSession::NotifyTPadNoReconnect()
{
	ZeroMemory(&m_cMsg, sizeof(CMessage));
	m_cMsg.event = ev_CnsDisconnect_Nty;	
	u16 wRet = PostMsg( TYPE_CMESSAGE );

	PrtMsg( ev_CnsDisconnect_Nty, emEventTypeCnsSend, "cnc Exit, No reconnect");
	return wRet;
}
