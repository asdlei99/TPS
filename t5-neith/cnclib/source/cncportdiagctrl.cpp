...
void CCncPortDiagCtrl::BuildEventsMap()
{
	...
	REG_PFUN(ev_CnIsShowLogo_Ind, CCncPortDiagCtrl::OnCnIsShowLogoInd);
	REG_PFUN(ev_CnIsShowLogo_Nty, CCncPortDiagCtrl::OnCnIsShowLogoNty);
	REG_PFUN(ev_CnIsShowBanner_Ind, CCncPortDiagCtrl::OnCnIsShowBannerInd);
	REG_PFUN(ev_CnIsShowBanner_Nty, CCncPortDiagCtrl::OnCnIsShowBannerNty);

	REG_PFUN(ev_tppSetEncryptInfo_Ind, CCncPortDiagCtrl::OnEncrytInd);
	REG_PFUN(ev_tppSetEncryptInfo_Nty, CCncPortDiagCtrl::OnEncrytNty);

	//ftp telnet 配置信息
	REG_PFUN(ev_CnTelnetInfo_Nty, CCncPortDiagCtrl::OnCnTelnetInfoNty);
	REG_PFUN(ev_CnFtpInfo_Nty, CCncPortDiagCtrl::OnCnFtpInfoNty);
	REG_PFUN(ev_CnSetFtp_Rsp, CCncPortDiagCtrl::OnCnFtpInfoRsp);
...
void CCncPortDiagCtrl::OnLinkBreak(const CMessage& cMsg)
{
	...
	memset( &m_tTPLogoInfo, 0, sizeof(TTPLogoInfo) );
	memset( &m_tTPBannerInfo, 0, sizeof(TTPBannerInfo) );
...
void CCncPortDiagCtrl::OnCnIsShowLogoInd( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);
	BOOL bSuccess = *(BOOL*)( cTpMsg.GetBody());
	TTPLogo tTPLogo = *(TTPLogo*)( cTpMsg.GetBody() + sizeof(BOOL)  );

	if (bSuccess)//成功了更新 只有登录时才有nty
	{
		switch(tTPLogo.byIndex)
		{
		case 0:
			m_tTPLogoInfo.tFirsLogoInfo = tTPLogo.tLogoInfo;
			break;
		case 1:
			m_tTPLogoInfo.tSecondLogoInfo = tTPLogo.tLogoInfo;
			break;
		case 2:
			m_tTPLogoInfo.tThirdLogoInfo = tTPLogo.tLogoInfo;
			break;
		}
	}

	PrtMsg( ev_CnIsShowLogo_Ind, emEventTypecnstoolRecv , "bSuccess:%d ID:%d",bSuccess, tTPLogo.byIndex);
	PostEvent( UI_CNC_ISSHOWLOGO_IND, (WPARAM)bSuccess, (LPARAM)tTPLogo.byIndex );
}

void CCncPortDiagCtrl::OnCnIsShowLogoNty( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);
	m_tTPLogoInfo = *(TTPLogoInfo*)( cTpMsg.GetBody() );
	PrtMsg( ev_CnIsShowLogo_Nty, emEventTypecnstoolRecv , "ShowLogo_Nty");
	PostEvent( UI_CNC_ISSHOWLOGO_NTY, NULL, NULL  );
}

void CCncPortDiagCtrl::OnCnIsShowBannerInd( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);
	BOOL bSuccess = *(BOOL*)( cTpMsg.GetBody());
	TTPBanner tBanner = *(TTPBanner*)( cTpMsg.GetBody() + sizeof(BOOL)  );

	if (bSuccess)//成功了更新 只有登录时才有nty
	{
		switch(tBanner.byIndex)
		{
		case 0:
			m_tTPBannerInfo.tFirstBannerInfo = tBanner.tBannerInfo;
			break;
		case 1:
			m_tTPBannerInfo.tSeconfBannerInfo = tBanner.tBannerInfo;
			break;
		case 2:
			m_tTPBannerInfo.tThirdBannerInfo = tBanner.tBannerInfo;
			break;
		}
	}

	PrtMsg( ev_CnIsShowBanner_Ind, emEventTypecnstoolRecv , "bSuccess:%d ID:%d",bSuccess, tBanner.byIndex);
	PostEvent( UI_CNC_ISSHOWBANNER_IND, (WPARAM)bSuccess, (LPARAM)tBanner.byIndex );
}

void CCncPortDiagCtrl::OnCnIsShowBannerNty( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);
	m_tTPBannerInfo = *(TTPBannerInfo*)( cTpMsg.GetBody() );
	PrtMsg( ev_CnIsShowBanner_Nty, emEventTypecnstoolRecv , "ShowBanner_Nty");
	PostEvent( UI_CNC_ISSHOWBANNER_NTY, NULL, NULL );
}

u16 CCncPortDiagCtrl::CnIsShowLogoCmd( const TTPLogo& tTPLogo )
{
	CTpMsg *pcTpMsg = m_pSession->GetKdvMsgPtr();
	pcTpMsg->SetUserData(m_pSession->GetInst());
	pcTpMsg->SetBody(&tTPLogo, sizeof(TTPLogo));
	pcTpMsg->SetEvent(ev_CnIsShowLogo_Cmd);

	u16 wRet = m_pSession->PostMsg(TYPE_TPMSG);
	PrtMsg( ev_CnIsShowLogo_Cmd, emEventTypecnstoolSend, "ShowLogo_Cmd id:%d IsShow:%d", tTPLogo.byIndex, tTPLogo.tLogoInfo.bIsShowLogo);
	return wRet;
}

u16 CCncPortDiagCtrl::CnIsShowBannerCmd( const TTPBanner& tTPBanner )
{
	CTpMsg *pcTpMsg = m_pSession->GetKdvMsgPtr();
	pcTpMsg->SetUserData(m_pSession->GetInst());
	pcTpMsg->SetBody(&tTPBanner, sizeof(TTPBanner));
	pcTpMsg->SetEvent(ev_CnIsShowBanner_Cmd);

	u16 wRet = m_pSession->PostMsg(TYPE_TPMSG);
	PrtMsg( ev_CnIsShowBanner_Cmd, emEventTypecnstoolSend, "ShowBanner_Cmd id:%d IsShow:%d", tTPBanner.byIndex, tTPBanner.tBannerInfo.bIsShowBanner);
	return wRet;
}

const TTPLogoInfo& CCncPortDiagCtrl::GetLogoInfo() const
{
	return m_tTPLogoInfo;
}

const TTPBannerInfo& CCncPortDiagCtrl::GetBannerInfo() const
{
	return m_tTPBannerInfo;
}

//Encryt加密设置
u16 CCncPortDiagCtrl::EncrytCmd( const TTpEncryptKey& tEncryt )
{
	CTpMsg *pcTpMsg = m_pSession->GetKdvMsgPtr();  
	pcTpMsg->SetUserData( m_pSession->GetInst() );
	pcTpMsg->SetBody( &tEncryt, sizeof(TTpEncryptKey) );
	pcTpMsg->SetEvent( ev_tppSetEncryptInfo_Cmd );

	u16 wRet = m_pSession->PostMsg(TYPE_TPMSG);
	PrtMsg( ev_tppSetEncryptInfo_Cmd, emEventTypecnstoolSend, _T( "emTPEncryptType:%d, byKey:%s byLen:%d emQtKeyFrequency:%d" )
		, tEncryt.emTPEncryptType, tEncryt.byKey, tEncryt.byLen, tEncryt.emQtKeyFrequency);
	return wRet;
}

void CCncPortDiagCtrl::OnEncrytInd( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);

	TTpEncryptKey tEncryt = *(TTpEncryptKey*)( cTpMsg.GetBody() );
	BOOL bSuc = *(BOOL*)( cTpMsg.GetBody() + sizeof(TTpEncryptKey));
    if ( bSuc )
    {
	    m_tEncryptType = tEncryt;
    }

	PostEvent( UI_ENCRYPTINFO_IND, (WPARAM)bSuc, 0 );

	PrtMsg( ev_tppSetEncryptInfo_Ind, emEventTypecnstoolRecv, _T( "emTPEncryptType:%d byKey:%s byLen:%d emQtKeyFrequency:%d IsSuc:%d" )
		, tEncryt.emTPEncryptType, tEncryt.byKey, tEncryt.byLen, tEncryt.emQtKeyFrequency, bSuc );
}


u16 CCncPortDiagCtrl::OnEncrytNty( const CMessage& cMsg )
{
	CTpMsg cTpMsg(&cMsg);
	...
	PostEvent( UI_ENCRYPTINFO_NTY, 0, 0 );

	PrtMsg( ev_tppSetEncryptInfo_Nty, emEventTypecnstoolRecv, _T( "emTPEncryptType:%d, byKey:%s byLen:%d emQtKeyFrequency:%d" )
		, m_tEncryptType.emTPEncryptType, m_tEncryptType.byKey ,m_tEncryptType.byLen, m_tEncryptType.emQtKeyFrequency);

	return NO_ERROR;
}

u16 CCncPortDiagCtrl::GetEncrytInfo( TTpEncryptKey& tEncryt )
{
	tEncryt = m_tEncryptType;
	return NO_ERROR;
}

TTPFtpInfo CCncPortDiagCtrl::GetCnFtpInfo()
{
	return m_tTPFtpInfo;
}

TTPTelnetInfo CCncPortDiagCtrl::GetCnTelnetInfo()
{
	return m_tTPTelnetInfo;
}

void CCncPortDiagCtrl::OnCnFtpInfoNty(const CMessage& cMsg)
{
	CTpMsg cTpMsg(&cMsg);
...
u16 CCncPortDiagCtrl::SetCnFtpInfo(const TTPFtpInfo tTPFtpInfo)
{
    CTpMsg *pcTpMsg = m_pSession->GetKdvMsgPtr();
    pcTpMsg->SetUserData( m_pSession->GetInst() );
...
void CCncPortDiagCtrl::OnCnFtpInfoRsp(const CMessage& cMsg)
{
	CTpMsg cTpMsg(&cMsg);
...
void CCncPortDiagCtrl::OnCnTelnetInfoNty(const CMessage& cMsg)
{
	CTpMsg cTpMsg(&cMsg);
	m_tTPTelnetInfo = *(TTPTelnetInfo*)( cTpMsg.GetBody() ); 
	PrtMsg( ev_CnTelnetInfo_Nty, emEventTypecnstoolRecv, "bOpen:%d", m_tTPTelnetInfo.bOpen );
}
