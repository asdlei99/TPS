...
public:
	...
	//Encryt密码信息
	virtual	u16 GetEncrytInfo( TTpEncryptKey& tEncryt );
	...
	* @remarks 
	*/	
	virtual const TTPLogoInfo& GetLogoInfo()const;
	/** 
	* 功能  	得到横幅信息
	* @param [in] 	   
	* @return  
	* @remarks 
	*/	
	virtual const TTPBannerInfo& GetBannerInfo()const;
    /** 
	* 功能  	设置ftp信息
	* @param [in] 	 
	* @return  
	* @remarks 
	*/	
	virtual u16 SetCnFtpInfo(TTPFtpInfo tTPFtpInfo);
	/** 
	* 功能  	获取ftp信息
	* @param [in] 	 
	* @return  
	* @remarks 
	*/	
	virtual TTPFtpInfo GetCnFtpInfo();
	...
	* @remarks 
	*/	
	virtual TTPTelnetInfo GetCnTelnetInfo();
...
protected:
	...
	void OnCnIsShowLogoInd(const CMessage& cMsg);
	void OnCnIsShowLogoNty(const CMessage& cMsg);
	void OnCnIsShowBannerInd(const CMessage& cMsg);
	void OnCnIsShowBannerNty(const CMessage& cMsg);

	//Encryt密码信息
	u16 OnEncrytNty( const CMessage& cMsg  );
	void OnEncrytInd( const CMessage& cMsg );
	...
	//telnet账号密码获取
	void OnCnTelnetInfoNty(const CMessage& cMsg);
...
private:
	...
	TTPLogoInfo	       m_tTPLogoInfo;
	TTPBannerInfo	   m_tTPBannerInfo;
	TTpEncryptKey      m_tEncryptType;//Encrypt密码设置
	//ftp和telnet配置信息
