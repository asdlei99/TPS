...
public:
	...
	// 设置连接的CNS实例号
	void SetInsID( u16 wDstInsID );
	u16  GetInsID();
	// 设置连接的APPID号
	void SetAppID( u16 wAppID );
	u16  GetAppID();
    //设置目的IID
    void SetDesIID( u32 dwDesID );
    u32  GetDesIID();
public:
    static CCncMsgDriver *s_pMsgDriver;
...
private:
    ...
    s32             m_nTimer;
	u16             m_wInsID;           // 连接的CNS实例号
	u16				m_wAppID;			// 连接的APPID号
    u32             m_dwDesIID;         // 目的IID
    // 消息分类映射表
    enum CATALOG{
        DEFAULT = 0,
        MULTI_CONF,
