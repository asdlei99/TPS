...
public:
	...
	TLoginRet GetLoginRet(){ return m_tLogRe; }

	//获得用户信息 dyy 2015-10-9
	CLoginRequest GetTpadLoginInfo(){ return m_cUser; }
    ...
    virtual u16 SetTpadSysName();//系统名修改通知tpad更新

	//cnc点击退出，通知TPad不再重连
	virtual u16 NotifyTPadNoReconnect();
...
protected:
	...
	//TPad 调出中控通知
	void OnTPadOpenCenterCtrl(const CMessage& cMsg);
	//TPad 调出系统设置通知
	void OnTPadOpenSysConfig( const CMessage& cMsg );
    //TPad 调出会场管理通知
    void OnTPadOpenCnsManage( const CMessage& cMsg );
	//白板会议状态通知
	void OnWBConfStateNty(const CMessage& cMsg);
	//开启数据会议提示
	void OnNewConfCallReq(const CMessage& cMsg);
	//TPad发来的cns断链通知
	void OnTPadCnsDisconnectNty(const CMessage& cMsg);
