...
//串口配置
u16 CCncPortDiagCtrl::SetSelectComCmd( const EmComType& emComType2, const EmComType& emComType3 ) const
{
	CTpMsg *pcTpMsg = m_pSession->GetKdvMsgPtr();  
	pcTpMsg->SetUserData( m_pSession->GetInst() );
	pcTpMsg->SetEvent( ev_Cn_SelectCom_Cmd);
	pcTpMsg->SetBody( &emComType2,sizeof( EmComType ) );
	pcTpMsg->CatBody( &emComType3,sizeof( EmComType ) );
	
	u16 wRet = m_pSession->PostMsg(TYPE_TPMSG);
	PrtMsg( ev_Cn_SelectCom_Cmd, emEventTypeCnsSend, "enComType2:%d, enComType3:%d ", emComType2, emComType3);
	return wRet;
}
