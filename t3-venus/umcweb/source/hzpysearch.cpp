...
BOOL  CHZPYSearch::HZPYMatchList( const char *strTextInput, const char* strTextSrc )
{
	...
	if ( nFind != -1 )
	{
		if ( 3 == CPrintCtrl::GetPrintCtrl()->GetPrintLevel() )
		{
			OspPrintf( TRUE, FALSE, "PinYin检索判断，strTextInput = %s, strTextSrc = %s\n", str1, str2 );
		}

		return TRUE;
	}

	CNode* pHead = CreateNodeList( (LPCTSTR)str2 );
	if ( NULL == pHead )
	{	
		return FALSE;
	}
...
CNode* CHZPYSearch::CreateNodeList( const char *strTextParm )
{
	...
	/*const*/ int length = strlen(strTextParm);
	...
	for ( int i = 0; i < nLen;  )
	{
		if ( strText[i] & 0x80 ) //汉字
		{
			...
			pHead = new CNode( '\0', pHead );  //new CNode( '0', pHead )---搜索'0'出错
...
bool CHZPYSearch::SearchHZPY( char *hzstr, char arPY[MAX_FIND_HANZI_LEN][MAX_FIND_HANZI_PY_CNT])
{
	...
	for( int k = 0; k<len; )
	{
		if ( hzstr[k] & 0x80 )//汉字
		{
			...
			CString strLetter;
            CListChild::GetFirstLetter( strKey, strLetter );
			strLetter.MakeLower();			
...
/*
功能：判断名字首字符对应的字母是否大于或等于指定字符
参数: strName - 传入的字串
	  selKey- 比较的字符
	  equalLetter - 为false则>=返回true,否则＝时返加true

BOOL CHZPYSearch::GetCurrLetter(CString strName, char selKey,bool equalLetter)
{
	TBYTE ucHigh, ucLow;
    int nCode;
    CString strRet;
...
//     strFirstLetter.Empty();
	...
	if(strName.GetLength()>0)
	{
		...
		{
			CString str=strName.Left(1);
			str.MakeUpper();
			if((BYTE)*str.GetBuffer(1)==(BYTE)selKey)
				return TRUE;
		}
		else
		{
			...
			if ( ucHigh >= 0xa1 && ucLow >= 0xa1)
			{
				...
				if(equalLetter)
				{
					if((BYTE)*strRet.GetBuffer(1)==(BYTE)selKey)
					return TRUE;
				}
				else
				{
