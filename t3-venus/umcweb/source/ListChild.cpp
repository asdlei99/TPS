// ListChild.cpp: implementation of the CListChild class.
//
//////////////////////////////////////////////////////////////////////
...
BEGIN_MESSAGE_MAP(CListChild, CTransparentList)
...
//////////////////////////////////////////////////////////////////////
CListChild::CListChild( CString strName , CRect rcWnd , CWnd* pWnd, CString strUI, UINT nID, 
					   DWORD dStyle , DWORD dwExStyle ) :
CTransparentList(strName, rcWnd, pWnd, strUI, nID,dStyle, dwExStyle )
{
	SetBkgColor(Color(239,245,255));
}

CListChild::~CListChild()
{

}


CString CListChild::FirstLetter(int nCode)
{
...
void CListChild::GetFirstLetter(CString  strName, CString& strFirstLetter)
{
    TBYTE ucHigh, ucLow;
    int  nCode;
    CString strRet;
...
void CListChild::SetData( const IValue& value )
{   
//	DWORD dwStart = GetTickCount();
	Clear();
	bool bRet = CTouchListHelp::SetData( GetRoot(), &value );
	if ( bRet == true )
	{ 
		LayoutItem();	
		ResetSelItem(); 
...
// 	CString str;
// 	str.Format("\n[SetData] setdataTime = %ld \n", dwEnd);
// 	AfxOutputDebugString( str);
}

void CListChild::Clicked( UINT nFlags, CPoint point )
{
	if ( m_pSelItem == 0 )
	{
		return;
	...
	if ( bFind == true )
	{
		...
		CMsgDispatch::ExeFunc( strFunc, clickInfo );
	}	
	else
	{
...
LRESULT CListChild::OnLetterClick( WPARAM wParam, LPARAM lParam )
{
...
bool CListChild::AddrItemCompare(TAddrItem& tItem1, TAddrItem& tItem2)
{   
	if ( tItem1 == tItem2 )
	{
		return true;
...
bool CListChild::UserItemCompare(CUserFullInfo& tItem1, CUserFullInfo& tItem2)
{
    CString strLetter1;
    CString strLetter2;	
...
bool CListChild::CnsInfoCompare(TCnsInfo& tInfo1, TCnsInfo& tInfo2)
{
	if ( tInfo1 == tInfo2 )
	{
		return true;
...
bool CListChild::TPCnsInfoCompare(TTPCnsInfo& tInfo1, TTPCnsInfo& tInfo2)
{ 
	CString strLetter1;
	CString strLetter2;	
...
bool CListChild::TempCnsCompare(TEpAddr  &tCns1, TEpAddr  &tCns2)
{
...
bool CListChild::ConfTempCompare( TTPAlias& tAlias1, TTPAlias& tAlias2 )
{   
    CString strLetter1;
    CString strLetter2;	
...
bool CListChild::OrderConfTempCompare( TTPConfTemplate& tTmpName1, TTPConfTemplate& tTmpName2 )
{   
// 	if ( tTmpName1 == tTmpName2 )
// 	{
// 		return true;
// 	}
	...
	GetFirstLetter( tTmpName1.m_achConfName, strLetter1 );
	GetFirstLetter( tTmpName2.m_achConfName, strLetter2 );
...
bool CListChild::ConfTemplateCompare( vector<TCMSConfTemplate>* pAryConfTemplate, BOOL bAscend )
{
    ...
    for (int i = 0; i < count-1; i++)
	{
		...
		{
			GetFirstLetter( pAryConfTemplate->at(j).m_achConfName, strLetter1 );
            GetFirstLetter( pAryConfTemplate->at(j + 1 ).m_achConfName, strLetter2 );
            if (bAscend)
            {                 
                // 升序
                if ( strLetter1.Compare( strLetter2 ) > 0 )
                {
                    TTPConfTemplate temp = pAryConfTemplate->at(j);
                    temp.m_dwTimeID = pAryConfTemplate->at(j).m_dwTimeID;
                    temp.m_wID = pAryConfTemplate->at(j).m_wID;
					
                    pAryConfTemplate->at(j) = pAryConfTemplate->at(j + 1 );
                    pAryConfTemplate->at(j).m_dwTimeID = pAryConfTemplate->at(j + 1 ).m_dwTimeID;
                    pAryConfTemplate->at(j).m_wID = pAryConfTemplate->at(j + 1 ).m_wID;
					
                    pAryConfTemplate->at(j + 1 ) = temp;
                    pAryConfTemplate->at(j + 1 ).m_wID =  temp.m_wID;
                    pAryConfTemplate->at(j + 1 ).m_dwTimeID = temp.m_dwTimeID ;
...
bool CListChild::CurConfCompare( TCMSConf& tConfName1, TCMSConf& tConfName2 )
{   
	if ( tConfName1 == tConfName2 )
	{
		return true;
	}
	...
	GetFirstLetter( tConfName1.m_achConfName, strLetter1 );
	GetFirstLetter( tConfName2.m_achConfName, strLetter2 );
...
bool CListChild::ConfMenuCompare(TConfMenuInfo& tItem1, TConfMenuInfo& tItem2)
{    
...
bool CListChild::VtrFileListItemCompare(TRecFileInfo& tItem1, TRecFileInfo& tItem2)
{   
	CString strItem1 = tItem1.m_szFilelName;
	CString strItem2 = tItem2.m_szFilelName;
	
	if ( strItem1.Compare( strItem2 ) < 0 || strItem1.Compare( strItem2 ) == 0)
	{
		return true;
	}
	else
...
bool CListChild::RecConfCompare(TUmcConfInfo& tItem1, TUmcConfInfo& tItem2)
{   
	if ( tItem1 == tItem2 )
	{
		return true;
	...
	CString strConfName1 = tItem1.m_achConfName;
	CString strConfName2 = tItem2.m_achConfName;

	if ( strConfName1.Compare( strConfName2 ) < 0 )
	{
		return true;
	}
	else
...
bool CListChild::RecMtCompare(TUmcRecInfo& tItem1, TUmcRecInfo& tItem2)
{
	CString strConfName1 = tItem1.m_achName;
	CString strConfName2 = tItem2.m_achName;
...
CViewList::CViewList( CString strName, CRect rcWnd, CWnd* pWnd, UINT nID, DWORD dwStyle, DWORD dwExStyle )
:CTransparentViewList( strName, rcWnd, pWnd, nID, dwStyle, dwExStyle )
{
...
void CViewList::SetData( const IValue& value )
{   
    Clear();
    bool bRet = CTouchListHelp::SetData( GetRoot(), &value );
    if ( bRet == true )
...
void CViewList::Clicked( UINT nFlags, CPoint point )
{
    if ( m_pSelItem == 0 )
    {
        return;
