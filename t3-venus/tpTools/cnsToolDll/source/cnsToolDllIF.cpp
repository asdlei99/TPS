// cnsToolDllIF.cpp : main header file for the CNSTOOLDLL DLL
/*---------------------------------------------------------------------
* 文 件	名： cnsToolDllIF.cpp
...
{ 
	...
	{    
		...
		//初始化lib
		LIBDATAMGRPTR->InitDataMgr();
		
		
		//初始化uifactory
		UIFACTORYMGR_PTR->InitUIFactory("rescnstool\\system.xml");
...
CNSTOOLDLL_API UINT _stdcall ConnectCns( DWORD dwCnsIP, DWORD dwCnsPort, CString strUserName, CString strPswd )
{
	AFX_MANAGE_STATE( AfxGetStaticModuleState());
	//调用common中的链接接口登陆cns 	 JYY  2013.4.2  启动线程来连接cns,连接结果直接由mainframe界面接收
 	/*u16 wRet = */COMIFMGRPTR->connect(dwCnsIP, dwCnsPort);
...
API void showver()
{   
    PrtMsg( "cnstool version: %s ", GetCnsToolBuildVersion() ); 
