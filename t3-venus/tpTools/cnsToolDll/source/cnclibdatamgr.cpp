...
bool CCnclibDataMgr::CheckVerNew()
{
	...
	achVerInfo[6] = '\0';
	int nNumVer = strcmp(achVerInfo, SOFTWARE_BOUNDARY_VERSION);
	//B6一下版本没有配置密码 ; B6自身及以上版本有配置账号密码
	bool bUseNew = false;
	if ( nNumVer >= 0 )
	{
		bUseNew = true;
...
u16 CCnclibDataMgr::GetDCamStateMap( map<u8, TCentreDCamCfg> &mapDCamCfg ) const
{
	CNCSESSION_MGR_PTR->GetCenterCtrlIF()->GetDCamStateMap(mapDCamCfg);
	return NO_ERROR;
}

u16 CCnclibDataMgr::GetDCamState( TCentreDCamCfg &tCentreDCamCfg, u8 byIndex ) const
{
	CNCSESSION_MGR_PTR->GetCenterCtrlIF()->GetDCamState(tCentreDCamCfg, byIndex);
	return NO_ERROR;
}

u16 CCnclibDataMgr::GetMatrixConfig(TTPMatrixConfig &tTPMatrixConfig) const
{
