...
void CTpToolsDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	//DDX_Control(pDX, IDC_RADIO_MOON, m_btnRadioMoon);
...
BOOL CTpToolsDlg::InitControl()
{
	...
	CString strVersion = "             2012-2019 苏州科达科技股份有限公司 V";
	strVersion += KDV_NMS_CNSTOOL_VER_PREFIX;
...
{ 
	...
	if ( NULL != m_pComboWnd )
	{
		...
		if ( m_btnRadioUms.GetCheck() )		//umstool tab
		{
			g_emServerType = em_UMSTOOL_APP;
			
			if ( !UMSTOOLSPACE::IsInitUmsTool() )
			{
				UMSTOOLSPACE::InitUmsTool(AfxGetMainWnd()->GetSafeHwnd());
			}
			UMSTOOLSPACE::ConnectToUms( strIP );  //点击登录   创建并连接umstool
		}
		else if (  m_btnRadioCns.GetCheck() )                           //cnstool tab
		{
			g_emServerType = em_CNSTOOL_APP;
			if ( !CNSTOOL::IsInitCnsTool() )
			{
				CNSTOOL::InitCnsTool( AfxGetMainWnd()->GetSafeHwnd() );
			}  
			CNSTOOL::ConnectCns( htonl(inet_addr(strIP)));
		}
		/*else
		{
...
BOOL CTpToolsDlg::CheckIPAddr(CString& strIp)
{
	...
	m_pComboWnd->IsIpInvalid(bIsEmpty );
	//	if( strIPAddr.GetLength() == 0 )
...
{
	if( strErrorText == "该IP正在抓包，拒绝抢登" )
	{
		ShowWarningText("该IP正在操作抓包，拒绝抢登");
	}
	else
	{
		CString strRet;
...
BOOL CTpToolsDlg::LoginRefuse(u32 dwIP)
{
	CString strIP;
	struct in_addr addrIPAddr;
	addrIPAddr.S_un.S_addr =  dwIP;
	...
	CString strIpAddr;
	strIpAddr.Format( "该IP正在操作抓包，拒绝抢登", strIP );
	ShowWarningText( strIpAddr );
	m_bRefuseLogin = TRUE;
	if ( !this->IsWindowVisible() )
	{
