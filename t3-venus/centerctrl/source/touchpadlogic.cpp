...
//		UIFACTORYMGR_PTR->SetTransStaticImage( _T("TouchPadDlg/StcAuxiLight"), g_strCurtainDownOffBkg, m_pWndTree );
    ...
    ComInterface->GetSysVersion( osvi );
    //xp以上版本才有触摸事件
    if ( osvi.dwMajorVersion > 5 )
    {  
        //注册响应WM_TOUCH的窗口
        Window* pWnd = UIFACTORYMGR_PTR->GetWindowPtr( "BtnSysSwitch", m_pWndTree ); 
        Window* pWnd1 = UIFACTORYMGR_PTR->GetWindowPtr( "StcBtn2", m_pWndTree ); 
        Window* pWnd2 = UIFACTORYMGR_PTR->GetWindowPtr( "StcBtn3", m_pWndTree ); 
        Window* pWnd3 = UIFACTORYMGR_PTR->GetWindowPtr( "StcBtn4", m_pWndTree ); 
